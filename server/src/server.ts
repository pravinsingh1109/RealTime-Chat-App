...
async function start(): Promise<void> {
  ...
  server.listen(env.PORT, () => {
    console.info(`API and Socket.io listening on port ${env.PORT}`);
    if (!databaseReady) {
      console.warn('Server started without a database connection. Some routes will be unavailable until MongoDB is reachable.');
    }
  });
