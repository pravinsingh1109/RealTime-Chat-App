...
export async function createChatMessage(input: CreateMessageInput): Promise<MessageDocument> {
  ...
  if (kind === 'image' && input.imageUrl && !input.imageUrl.startsWith('/uploads/')) {
    throw new ApiError(400, 'Image messages must reference an uploaded image.', 'INVALID_IMAGE_URL');
  }
...
export async function markConversationRead(input: MarkReadInput): Promise<{ readAt: Date; messageId?: string }> {
  ...
  if (input.messageId) {
    ...
    if (!targetMessage) {
      throw new ApiError(404, 'Message not found in this conversation.', 'MESSAGE_NOT_FOUND');
    }
