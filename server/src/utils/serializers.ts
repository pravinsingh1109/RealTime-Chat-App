...
export function serializeId(value: unknown): string {
  ...
  if (value && typeof value === 'object') {
    ...
    // Mongoose/BSON ObjectId has toHexString()
    if (typeof anyv.toHexString === 'function') {
      try {
        return anyv.toHexString();
      } catch {
        // fallthrough
      }
    }

    // Plain object with nested _id or id
    const tag = Object.prototype.toString.call(anyv);
    if (tag === '[object Object]') {
      if ('_id' in anyv) return serializeId(anyv._id);
      if ('id' in anyv) return serializeId(anyv.id);
    }
